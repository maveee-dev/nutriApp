...
export function createCkdTargetRegistrations(): readonly NutritionTargetPolicyRegistration[] {
  ...
  return [
    simple(CKD_PHOSPHORUS_POLICY_ID, CKD_PHOSPHORUS_POLICY_VERSION, (context) => {
      ...
      precedenceByConflictKey: { [TARGET_KEYS.phosphorus]: 25 }, evaluationRule: {
        family: 'numeric-constraint', kind: 'upper-limit', roles: ['compatibility', 'contribution', 'progress'], scopes: ['food', 'meal', 'daily'],
        measurementKey: 'phosphorus', unit: 'mg', weight: 30,
      },
    }),
    simple(CKD_POTASSIUM_POLICY_ID, CKD_POTASSIUM_POLICY_VERSION, (context) => {
      ...
      precedenceByConflictKey: { [TARGET_KEYS.potassium]: 25 }, evaluationRule: {
        family: 'numeric-constraint', kind: 'upper-limit', roles: ['compatibility', 'contribution', 'progress'], scopes: ['food', 'meal', 'daily'],
        measurementKey: 'potassium', unit: 'mg', weight: 30,
      },
    }),
    simple(CKD_NON_DIALYSIS_PROTEIN_POLICY_ID, CKD_NON_DIALYSIS_PROTEIN_POLICY_VERSION, (context, candidates) => {
      ...
      dependsOn: ['general-protein-baseline-v1'], precedenceByConflictKey: { [TARGET_KEYS.protein]: 20 }, evaluationRule: {
        family: 'numeric-constraint', kind: 'lower-target', roles: ['contribution', 'progress'], scopes: ['food', 'meal', 'daily'],
        measurementKey: 'protein', unit: 'g', weight: 30,
      },
    }),
    simple(HEMODIALYSIS_PROTEIN_POLICY_ID, HEMODIALYSIS_PROTEIN_POLICY_VERSION, (context, candidates) => {
      ...
      dependsOn: ['general-protein-baseline-v1'], precedenceByConflictKey: { [TARGET_KEYS.protein]: 30 }, evaluationRule: {
        family: 'numeric-constraint', kind: 'lower-target', roles: ['contribution', 'progress'], scopes: ['food', 'meal', 'daily'],
        measurementKey: 'protein', unit: 'g', weight: 30,
      },
    }),
